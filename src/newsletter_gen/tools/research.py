...
class SearchAndContents(BaseTool):
    ...
    def _run(self, search_query: str, start_datetime: str, end_datetime: str) -> str:
        ...
        try:
            search_results = exa.search_and_contents(
                query=search_query,
                use_autoprompt=True,
                start_published_date=start_datetime,
                end_published_date=end_datetime,
...
class FindSimilar(BaseTool):
    ...
    def _run(self, article_url: str, start_datetime: str, end_datetime: str) -> str:
        exa = Exa(api_key=os.getenv("EXA_API_KEY"))
...
class GetContents(BaseTool):
    ...
    def _run(self, article_ids: list, start_datetime: str, end_datetime: str) -> str:
        exa = Exa(api_key=os.getenv("EXA_API_KEY"))

        try:
            contents = exa.get_contents(article_ids)
            
            # Filter contents based on date range
            filtered_contents = [
                content for content in contents 
                if start_datetime <= content['published_date'] <= end_datetime
            ]
